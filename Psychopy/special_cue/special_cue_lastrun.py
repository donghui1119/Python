...
expName = 'special_cue'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='F:\\psychopy\\special_cue\\special_cue_lastrun.py',
...
fixationITI = visual.ShapeStim(
    win=win, name='fixationITI', vertices='cross',units='deg', 
    size=(0.5, 0.5),
    ori=0, pos=(0, 0),
...
trials = data.TrialHandler(nReps=2, method='random', 
    ...
    trialList=data.importConditions('cue_target.xlsx'),
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if fixationITI.status == STARTED:
            ...
            if tThisFlipGlobal > fixationITI.tStartRefresh + random()+1-frameTolerance:
                ...
                win.timeOnFlip(fixationITI, 'tStopRefresh')  # time at next scr refresh
    ...
    trials.addData('fixationITI.started', fixationITI.tStartRefresh)
    trials.addData('fixationITI.stopped', fixationITI.tStopRefresh)
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [cue, target, key_resp]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if cue.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > cue.tStartRefresh + 12-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *key_resp* updates
        waitOnFlip = False
        if key_resp.status == NOT_STARTED and frameN >= 12:
            # keep track of start time/frame for later
