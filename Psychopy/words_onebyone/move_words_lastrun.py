...
expName = 'move_words'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='F:\\psychopy\\move_words_lastrun.py',
...
text = visual.TextStim(win=win, name='text',
    text='When will participants get the meaning of the sentence?',
    font='Arial',
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if mask.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > mask.tStartRefresh + 4-frameTolerance:
            # keep track of stop time/frame for later
